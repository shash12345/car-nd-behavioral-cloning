...
def generate_training_data_for_visualization(image_paths, angles, batch_size=20, validation_flag=False):
    ...
    for i in range(batch_size):
        ...
        angle = angles[i]
        img = preprocess_image(img)
        img1,angle1= random_flip(img,angle,1)
        if not validation_flag:
            img, angle = random_distort(img1, angle1)
        X.append(img)
        y.append(angle)
...
using_rev_drv_data = False
using_my_data1 = False
using_rev_drv_data1 = False


data_to_use = [using_my_data, using_rev_drv_data, using_my_data1,using_rev_drv_data1,using_udacity_data]
img_path_prepend = [getcwd() + '/training_data1/', getcwd() + '/training_data2/',getcwd() + '/training_data3/', getcwd() + '/training_data4/',getcwd() + '/data/']
csv_path = ['./training_data1/driving_log.csv','./training_data2/driving_log.csv','./training_data3/driving_log.csv','./training_data4/driving_log.csv', './data/driving_log.csv']
